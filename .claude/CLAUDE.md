...
You operate fluently in: the Gmail REST API (v1) — messages, drafts, threads, labels, filters, settings.{vacation,imap,pop,language,autoForwarding,sendAs,forwardingAddresses,delegates,smimeInfo}, history, watch; Google Application Default Credentials (`gcloud auth application-default login`, the auto-discovered `~/.config/gcloud/application_default_credentials.json`, `google.auth.GoogleAuth({scopes})` in the `googleapis` Node library, transparent token refresh via `google-auth-library`); multipart MIME (mixed + alternative) including base64 encoding, quoted-printable, RFC 2047 header encoding, and threaded reply headers (Message-Id, In-Reply-To, References); the Model Context Protocol specification (tools/list and tools/call); Node.js + ES modules + the `googleapis` and `@modelcontextprotocol/sdk` libraries.
...
**When ADC or auth fails:**
- First action: `node bin/gmail-cli.js auth-status`. It walks the six gates (gcloud installed, ADC present, scope granted, quota project set, Gmail API enabled, live call works) and prints the exact fix command for the first failure.
...
- Token refresh is handled transparently by `google-auth-library`. There is no token cache for us to manage.
...
- Mike must re-run `gcloud auth application-default login --scopes=<full list>` (the FULL list, not just the new scope — gcloud overwrites). Tell him this; never silently invalidate.
- Bump the SCOPES check logic in `auth-status` if the new scope isn't a `gmail.googleapis.com/auth/*` flavor.
...
- **Deps**: `googleapis` (Gmail v1 client), `@modelcontextprotocol/sdk` (MCP server)
- **Auth**: Application Default Credentials. `gcloud auth application-default login --scopes=https://mail.google.com/` writes the refresh token to `~/.config/gcloud/application_default_credentials.json`. `googleapis` Node library auto-discovers it; refresh is transparent.
...
- `lib/auth.js` — ADC client factory + error mappers (mapAuthError, mapApiError)
...
- `bin/gmail-cli.js` — CLI dispatch + arg parsing + `auth-status` diagnostic + profile cache
- `bin/gmail-mcp.js` — MCP server with TOOLS map (one entry per CLI subcommand, JSON Schema validated)
- `SETUP.md` — two-command setup walkthrough (gcloud install + ADC login)
...
- **`https://mail.google.com/` full scope** — granular scopes balloon the OAuth consent UI without security benefit for a single-user personal-use desktop app. Tradeoff: any compromise of `~/.config/gcloud/application_default_credentials.json` (where ADC stores Mike's refresh token) is full mail access. ADC writes the file with restrictive perms by default.
