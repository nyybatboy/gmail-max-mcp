...
function quoteAsHtml(parentHtml, parentText) {
  if (parentHtml) {
    // Gmail-style nested blockquote with subtle left border.
    return `<blockquote class="gmail_quote" style="margin:0 0 0 .8ex;border-left:1px #ccc solid;padding-left:1ex">${parentHtml}</blockquote>`;
  }
  if (parentText) {
    const escaped = parentText
      ...
      .replace(/>/g, '&gt;')
      .replace(/\r?\n/g, '<br>');
    return `<blockquote class="gmail_quote" style="margin:0 0 0 .8ex;border-left:1px #ccc solid;padding-left:1ex">${escaped}</blockquote>`;
  }
  return '';
}
...
export async function createReplyDraft({
  ...
  if (includeQuotedParent) {
    ...
    if (parentText || parentHtml) {
      ...
      const userHtml = html || (body ? body.replace(/\r?\n/g, '<br>') : '');
      const quotedHtml = quoteAsHtml(parentHtml, parentText);
      finalHtml = `<div>${userHtml}</div><br><div class="gmail_attr">${replyHeader}<br></div>${quotedHtml}`;
    }
  }
