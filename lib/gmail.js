...
async function applySignature(opts) {
  ...
  // Only attach a signature to the html part when the caller supplied their own html.
  // When html is absent, leave it absent so buildRawMessage renders the combined
  // text (body + signature) into the HTML alternative at the single chokepoint —
  // otherwise the html part would contain ONLY the signature and drop the body.
  if (userHtml) {
    signed.html = `${userHtml}<br><br><div class="gmail_signature" data-smartmail="gmail_signature">${sig.html}</div>`;
  }
  return signed;
}
...
/**
 * Split a raw RFC 5322 address-list header value into individual address strings.
 * Respects double-quoted display names that may themselves contain commas
 * (e.g. `"Lastname, Firstname" <x@y.com>, other@z.com`).
 */
...
/** Extract the bare email address (lowercased) from a single address string. */
export function extractEmail(addr) {
  if (!addr) return '';
  const angle = String(addr).match(/<([^>]+)>/);
  const email = angle ? angle[1] : String(addr);
  return email.trim().toLowerCase();
}

/**
 * Compute { to, cc } for a reply from the parent message's headers.
 *
 * Two cases:
 *  - Parent sent by the user (self-sent, e.g. bumping a thread): the user is NOT a
 *    recipient of their own reply, so we address To: the parent's original To
 *    recipients; on replyAll we carry the parent's Cc as Cc.
 *  - Parent sent by someone else (normal): To: the parent's Reply-To/From; on
 *    replyAll we add the parent's To + Cc.
 *
 * In both cases the user's own address — the authenticated account or ANY send-as
 * alias (selfAddresses) — is filtered out of every recipient field, and Cc is
 * de-duped against To. Caller-supplied extraTo/extraCc are appended verbatim (the
 * caller asked for them explicitly, so they are never self-filtered).
 *
 * Pure function: no I/O. selfAddresses is resolved by the caller (getSelfAddresses).
 */
export function buildReplyRecipients({ headers = {}, replyAll = false, selfAddresses = [], extraTo, extraCc } = {}) {
  const self = new Set(
    (Array.isArray(selfAddresses) ? selfAddresses : [selfAddresses])
      .map(a => String(a || '').trim().toLowerCase())
      .filter(Boolean),
  );
  const isSelf = (addr) => self.has(extractEmail(addr));

  const from = headers['from'] || '';
  const replyTo = headers['reply-to'] || from;
  const parentFromIsSelf = from ? isSelf(from) : false;

  const parentTo = splitAddressList(headers['to']);
  const parentCc = splitAddressList(headers['cc']);

  const extraToArr = extraTo ? (Array.isArray(extraTo) ? extraTo : [extraTo]) : [];
  const extraCcArr = extraCc ? (Array.isArray(extraCc) ? extraCc : [extraCc]) : [];

  let to;
  let cc = [];
  if (parentFromIsSelf) {
    to = [...parentTo];
    if (replyAll) cc = [...parentCc];
  } else {
    to = splitAddressList(replyTo);
    if (replyAll) cc = [...parentTo, ...parentCc];
  }

  // Never address the user's own account/aliases in their own reply.
  to = to.filter(a => !isSelf(a));
  cc = cc.filter(a => !isSelf(a));

  // Drop any Cc that already appears in To, then de-dupe Cc against itself.
  const toEmails = new Set(to.map(extractEmail));
  const seenCc = new Set();
  cc = cc.filter(a => {
    const e = extractEmail(a);
    if (!e || toEmails.has(e) || seenCc.has(e)) return false;
    seenCc.add(e);
    return true;
  });

  return { to: [...to, ...extraToArr], cc: [...cc, ...extraCcArr] };
}

/**
 * Resolve the user's own addresses: the authenticated Gmail account plus every
 * send-as alias. Best-effort — a transient failure on either lookup degrades to
 * whatever addresses we did resolve rather than making reply drafting fragile.
 */
async function getSelfAddresses() {
  const addrs = new Set();
  try {
    const profile = await getProfile();
    if (profile?.emailAddress) addrs.add(profile.emailAddress.toLowerCase());
  } catch { /* best effort */ }
  try {
    const data = await listSendAs();
    for (const sa of data?.sendAs || []) {
      if (sa.sendAsEmail) addrs.add(sa.sendAsEmail.toLowerCase());
    }
  } catch { /* best effort */ }
  return [...addrs];
}
 ...
 *
 * Addressing: see buildReplyRecipients. Replying to a message the user sent
 * themselves addresses the original recipients, never the user.
 */
export async function createReplyDraft({
  toMessageId, replyAll = false, body, html, attachments, extraTo, extraCc, extraBcc, fromAlias,
  includeQuotedParent = true, useSignature = true, plaintextOnly = false,
}) {
  ...
  // Address the reply. buildReplyRecipients handles the normal-vs-self-sent
  // branching and excludes the user's own account/aliases from every field, so a
  // reply to one of the user's own messages goes to the original recipients, not
  // back to the user. selfAddresses = authenticated account + all send-as aliases.
  const selfAddresses = await getSelfAddresses();
  const { to, cc } = buildReplyRecipients({ headers, replyAll, selfAddresses, extraTo, extraCc });
  ...
  return createDraft({
    ...
    text: finalText,
    html: plaintextOnly ? undefined : finalHtml,
    plaintextOnly,
    attachments,
