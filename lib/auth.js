...
function readCredentials() {
  if (!fs.existsSync(CREDENTIALS_PATH)) {
    throw new Error(
      ...
      `Fix: create a Desktop OAuth client in Google Cloud Console (you must own the project ` +
      `and add yourself as a test user on the consent screen — Gmail's full-access scope is ` +
      `restricted, so gcloud's pre-built OAuth client cannot grant it). Download the JSON to ` +
      `${CREDENTIALS_PATH}. See SETUP.md for the four-click walkthrough.`
    );
  }
