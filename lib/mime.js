 ...
 * @param {Array}  [m.inlineImages]           [{ path|contentBase64, cid, filename?, mimeType? }] — inline via Content-ID
...
export function buildRawMessage(m) {
  ...
  const boundaryRel = `rel_${crypto.randomBytes(8).toString('hex')}`;
  const hasAttachments = m.attachments && m.attachments.length > 0;
  const hasInline = m.inlineImages && m.inlineImages.length > 0;
  ...
  function relatedContent() {
    ...
    const parts = [];
    parts.push(`--${boundaryRel}`);
    parts.push(`Content-Type: ${inner.contentTypeHeader}`);
    if (inner.transferEncoding) parts.push(`Content-Transfer-Encoding: ${inner.transferEncoding}`);
    parts.push('');
    parts.push(inner.body);
    for (const img of m.inlineImages) {
      const a = readAttachment(img);
      const cid = img.cid || crypto.randomBytes(8).toString('hex');
      parts.push(`--${boundaryRel}`);
      parts.push(`Content-Type: ${a.mimeType}; name="${a.filename}"`);
      parts.push(`Content-Disposition: inline; filename="${a.filename}"`);
      parts.push(`Content-Transfer-Encoding: base64`);
      parts.push(`Content-ID: <${cid}>`);
      parts.push('');
      parts.push(chunk(a.data));
    }
    parts.push(`--${boundaryRel}--`);
    return {
      contentTypeHeader: `multipart/related; boundary="${boundaryRel}"`,
      body: parts.join('\r\n'),
    };
  }

  function topBodyContent() {
    return hasInline ? relatedContent() : bodyContent();
  }

  if (hasAttachments) {
    headers.push(`Content-Type: multipart/mixed; boundary="${boundaryMixed}"`);
    headers.push('');
    const top = topBodyContent();
    bodyParts.push(`--${boundaryMixed}`);
    bodyParts.push(`Content-Type: ${top.contentTypeHeader}`);
    if (top.transferEncoding) bodyParts.push(`Content-Transfer-Encoding: ${top.transferEncoding}`);
    bodyParts.push('');
    bodyParts.push(top.body);
    ...
    const top = relatedContent();
    headers.push(`Content-Type: ${top.contentTypeHeader}`);
    headers.push('');
    bodyParts.push(top.body);
  } else {
    const inner = bodyContent();
