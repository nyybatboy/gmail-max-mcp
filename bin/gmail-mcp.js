...
const MESSAGE_BODY = {
  ...
  properties: {
    ...
    subject: { type: 'string' },
    text: { type: 'string', description: 'Plaintext body. Write it the way a person types plainly into Gmail compose: one continuous line per paragraph, a blank line between paragraphs. It is automatically rendered into a multipart/alternative email with a Gmail-style HTML part (so it reflows on every screen) plus this text as the plain-text fallback. You do not need to write HTML.' },
  ...
  create_reply_draft: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        body:        { type: 'string', description: 'Plaintext reply body, written plainly (one line per paragraph, blank line between). Auto-rendered to a Gmail-style HTML reply with a plain-text fallback; you do not need to write HTML.' },
